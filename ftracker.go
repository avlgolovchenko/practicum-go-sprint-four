...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	if height == 0 {
		fmt.Println("error in meaning height")
		return 0
	}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	speedKmh := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (speedKmh + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
